...
describe("incrementRound mutation", () => {
  it("推进回合并按 36 回合/年从 2012 推导游戏内年份", () => {
    ...
    while (store.state.gameLoop.round < 37) store.commit("incrementRound");
    expect(store.state.gameLoop.round).toBe(37);
    expect(store.state.gameLoop.year).toBe(2013);
  });

  it("把金条利息计入金钱：money += ceil(gold * 0.06 * 552)", () => {
    store.commit("updateAttribute", { attribute: "gold", value: 10 });
    const before = store.state.character.attributes.money;
    store.commit("incrementRound");
    expect(store.state.character.attributes.money).toBe(before + Math.ceil(10 * 0.06 * 552));
  });
  ...
  it("仅在恋爱中推进 relationRound", () => {
    ...
    const before = store.state.relationship.relationRound;
    store.commit("incrementRound");
    expect(store.state.relationship.relationRound).toBe(before + 1);
  });
});
...
describe("updateAttribute mutation", () => {
  ...
  it("red / black 直接调整时下限钳制为 0", () => {
    ...
    expect(store.state.character.attributes.popularity.red).toBe(0);
    expect(store.state.character.attributes.popularity.black).toBe(0);
  });
  ...
  it("游戏技能被当前等级段上限钳制（D 段上限 3）", () => {
    ...
    expect(store.state.character.attributes.skill.gaming).toBe(dBand!.max);
  });

  it("体力上限钳制为 maxEnergy，低于 0 进入虚弱", () => {
    const maxEnergy = store.state.character.attributes.maxEnergy;
    store.commit("updateAttribute", { attribute: "energy", value: 9999 });
    expect(store.state.character.attributes.energy).toBe(maxEnergy);

    store.commit("updateAttribute", { attribute: "energy", value: -(maxEnergy + 5) });
    expect(store.state.character.attributes.energy).toBeLessThan(0);
...
describe("buyGold mutation", () => {
  it("按 552/根 增加金条、扣减金钱", () => {
    ...
    store.commit("character/buyGold", 2);
    expect(store.state.character.attributes.gold).toBe(2);
    expect(store.state.character.attributes.money).toBe(2000 - 552 * 2);
  });
});

describe("updateItem mutation", () => {
  it("普通物品按数量叠加", () => {
    store.commit("progress/updateItem", { itemName: "皮卡丘玩偶", quantity: 5 });
    store.commit("progress/updateItem", { itemName: "皮卡丘玩偶", quantity: 3 });
    expect(store.state.progress.inventory["皮卡丘玩偶"].quantity).toBe(8);
  });

  it("特殊装备至多 1 件并记录 lastSpecialItem", () => {
    store.commit("progress/updateItem", { itemName: "麦克风大锤", quantity: 1 });
    store.commit("progress/updateItem", { itemName: "麦克风大锤", quantity: 1 });
...
describe("upgradeSkillLevel mutation", () => {
  it("累加技能点并重算等级段（4 点 -> C 段）", () => {
    for (let i = 0; i < 4; i++) store.commit("character/upgradeSkillLevel", "freestyle");
    expect(store.state.character.attributes.skill.freestyle).toBe(4);
    expect(store.state.character.attributes.skill.freestyleLevel).toBe("C");
  });
});
