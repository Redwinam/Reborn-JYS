...
describe("eatFood", () => {
  it("已解锁且有钱：扣除餐费、恢复体力", async () => {
    ...
    expect(store.state.character.attributes.energy).toBeGreaterThan(100);
  });

  it("钱不够：不扣钱、不回体力", async () => {
    store.commit("unlockFood", { name: "火锅", cost: 60, energy: 100, taste: "spicy" });
    const energyBefore = store.state.character.attributes.energy;
    await store.dispatch("eatFood", "火锅");
    expect(store.state.character.attributes.money).toBe(0);
    expect(store.state.character.attributes.energy).toBe(energyBefore);
  });
});
...
describe("drinkDrink", () => {
  it("喝啤酒：扣钱、心情-10、进入醉酒", async () => {
    ...
    expect(store.state.character.attributes.money).toBe(85);
    expect(store.state.character.attributes.mood).toBe(-10);
    expect(store.state.character.drunk).toBe(1);
  });
});
