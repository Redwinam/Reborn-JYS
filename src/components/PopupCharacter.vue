<template>
  <div class="attributes-container">
    ...
    <div>
      <span class="attribute-name">{{ attributeNames["talent"] }}</span> {{ attributes["talent"] }}
    </div>
    <div>
      <span class="attribute-name">{{ attributeNames["charm"] }}</span> {{ attributes["charm"] }}
    </div>
    <div>
      <span class="attribute-name">{{ attributeNames["divine"] }}</span> <span>{{ attributes["divine"] }}</span>
    </div>

    <div>
      <span class="attribute-name">{{ attributeNames["popularity"] }}</span> 红 {{ Math.round(attributes["popularity"]["red"]) }} / 黑 {{ Math.round(attributes["popularity"]["black"]) }}
    </div>
    <div>
    ...
    <div v-if="attributes['energy'] >= 0">
      <span class="attribute-name"> {{ attributeNames["energy"] }}</span> {{ attributes["energy"] }}<template v-if="weak">（虚弱！）</template>
    </div>
    <div v-else><span class="weak attribute-name">体力透支</span> {{ attributes["energy"] }} <template v-if="weak">（虚弱！）</template></div>
    <div>
      <span class="attribute-name">{{ attributeNames["mood"] }}</span> {{ attributes["mood"] }} <template v-if="drunk > 0">（醉酒 × {{ drunk }}）</template>
    </div>
  </div>
