...
const battleReward = async () => {
  ...
  if (result === '海选') {
    store.commit('updateAttributes', { attribute: 'popularity', value: reward * 0.5 })
    store.commit('updateAttributes', { attribute: 'money', value: reward * 10 * 0.5 })
    await store.dispatch('typeWriter', `本届Battle大赛告一段落，恭喜姜云升获得「海选」奖励${reward * 0.5}人气和${reward * 10 * 0.5}金钱！`)
  } else if (result === '八强') {
    store.commit('updateAttributes', { attribute: 'popularity', value: reward * 0.75 })
    store.commit('updateAttributes', { attribute: 'money', value: reward * 10 * 0.75 })
    await store.dispatch('typeWriter', `本届Battle大赛告一段落，恭喜姜云升获得「八强」奖励${reward * 0.75}人气和${reward * 10 * 0.75}金钱！`)
  } else if (result === '冠军') {
    store.commit('updateAttributes', { attribute: 'popularity', value: reward * 1 })
    store.commit('updateAttributes', { attribute: 'money', value: reward * 10 * 1})
