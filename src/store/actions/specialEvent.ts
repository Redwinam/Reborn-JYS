...
// Phase 3d：原 specialEventOptionChosen 是一个 300 行的巨型 if-else（按事件分支）。
// 现按事件拆成 per-event handler + 分发 map，各 handler 行为与原分支完全一致（纯机械抽取）。
type OptionPayload = { event: string; option: string };

async function onJiangGeWanWanHao(context: SpecialEventContext, payload: OptionPayload) {
...
async function onBirthday(context: SpecialEventContext, payload: OptionPayload) {
  console.log("生日快乐");
...
async function onWatchFun(context: SpecialEventContext, payload: OptionPayload) {
...
async function onTenYears(context: SpecialEventContext, payload: OptionPayload) {
...
async function onRelaxBreathe(context: SpecialEventContext, payload: OptionPayload) {
...
async function onWedding(context: SpecialEventContext, payload: OptionPayload) {
...
async function onRPSKing(context: SpecialEventContext, payload: OptionPayload) {
...
async function onSignAgency(context: SpecialEventContext, payload: OptionPayload) {
...
export async function specialEventOptionChosen(context: SpecialEventContext, payload: OptionPayload) {
  const handler = optionHandlers[payload.event];
  if (handler) {
