...
//（原先它们共享 else 块末尾的 incrementRound，提前 return 的分支会跳过——拆分后由各
// handler 自带末尾 incrementRound，内部 return 同样跳过，语义一致）。
type PerformActionContext = { commit: Commit; dispatch: Function; getters: any };

async function onGoOut(context: PerformActionContext) {
  if (store.state.character.attributes.energy >= 0) {
    isGoingOut.value = true;

    if (store.state.progress.songStages["致素未谋面却如此相似的我们"]?.completedStage === "release" && store.state.gameLoop.currentLyricIndex === -1) {
...
async function onGoHome(context: PerformActionContext) {
  isAtHome.value = true;
  await context.dispatch("typeWriter", "姜云升回到了家。");
}

async function onWriteSong(_context: PerformActionContext) {
  showSongWritingDialog.value = true;
}

async function onStudy(context: PerformActionContext) {
...
async function onMakeMoney(context: PerformActionContext) {
...
async function onHangOut(context: PerformActionContext) {
...
async function onSleep(context: PerformActionContext) {
...
async function onPlayGame(context: PerformActionContext) {
  const gamingIntros = [
    "姜云升一回家就开了一把《永劫有间》！",
...
async function onLiveStream(context: PerformActionContext) {
  if (store.state.character.attributes.energy < 0) {
